...
class cTimer
{
	...
	__forceinline float GetElapsedTime(){return elapsedTime;}
	//returns the time between two AdvanceTimer() calls
	__forceinline float GetDelay(){return delay;}
	//sets the elapsed time. Can be used in conjunction with AdvanceTimeNegative() as a countdown timer
	__forceinline void SetElapsedTime(float value){elapsedTime=value;}
	//pasues the timer
